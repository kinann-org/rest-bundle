...
<div>
    <rb-about v-if="about" :name="componentName">
        <p> Displays RestBundle service information returned by <code>GET /identity</code>. In addition,
            if the server provides periodic <code>RbSingleton.pushState()</code>, 
    ...
    <v-card flat >
        <v-card-text class="pl-5 ml-1">
            <v-layout row >
                <v-flex xs2><b>Service:</b></v-flex>
                <v-flex xs9> 
                    <slot>RestBundle service "{{service}}" </slot>
                </v-flex>
            </v-layout>
            <v-layout row >
